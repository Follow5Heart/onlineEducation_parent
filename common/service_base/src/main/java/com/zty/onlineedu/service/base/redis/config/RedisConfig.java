...
public class RedisConfig extends CachingConfigurerSupport {
     ...
     * **
     * 就是如果不进行自定义Spring缓存模块的错误处理器，那么会抛出异常，直接报错，到处程序中断
     * 如果自定义Spring缓存模块的错误处理器，尽管redis挂了，那么会走redis的容错处理，最后绕过redis直接走数据库
     * * *
     * 如果不添加这段代码，则Spring缓存模块默认的错误处理机制会被使用。默认情况下，
     * * Spring会在出现缓存异常时抛出异常，需要调用者自行处理。因此，添加这段代码可以在缓存操作出现异常时提供更加友好和实用的错误处理方式，
     * * 例如在Redis服务器宕机时可以自动切换到数据库操作以保证系统的正常运行
     * ========代码仅仅是用于处理缓存异常，并不会影响程序自动从数据库中访问数据的逻辑=========
     *
     * @return
     */
    @Bean
